...
function m6Context(PricingType $pricingType = PricingType::Fixed, string $suffix = ''): array
{
    ...
    app(ManagePaymentChannelsService::class)->setActive($superUser, 'NQ', true);
...
function m6FakeCreate(string $inquiryStatus = '01', ?int $inquiryFee = null): void
{
    ...
    Http::fake([
        (string) config('services.duitku.inquiry_url') => Http::response(['reference' => 'REF-TEST-1', 'statusCode' => $inquiryStatus, 'fee' => $inquiryFee], 200),
    ]);
}

function m6Invoice(array $context, string $inquiryStatus = '01', ?int $inquiryFee = null): PaymentData
{
    m6FakeCreate($inquiryStatus, $inquiryFee);
...
it('creates a fixed invoice with a unique merchant order id and sixty minute expiry', function () {
    ...
    expect($attempt->merchantOrderId)->toStartWith('KL-')
        ...
        ->and($attempt->expiresAt->format('Y-m-d H:i'))->toBe('2026-09-18 09:00')
...
it('applies controlled inquiry results and stores actual fees', function () {
    ...
    $result = app(InquirePaymentService::class)->handle($context['superUser'], $attempt->merchantOrderId);
...
it('expires overdue attempts through the scheduled job', function () {
    ...
    app(ExpirePaymentAttemptsJob::class)->handle(app(PaymentRepositoryInterface::class));
